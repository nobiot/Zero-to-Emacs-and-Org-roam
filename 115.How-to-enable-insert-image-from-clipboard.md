...
### [2021-02-06 update] Further improvement to the code -- promt to select directory

I further tweaked my functions to let me choose the image directory. It defaults to `./images` but you can traverse the directory tree up and down to specify the one you want to use. As of 2021-02-06, these are the functions I use for my personal use -- just as a reference for when you wish to extend the original functions presented above.

```emacs-lisp

(defun my/org-screenshot ()
  "Prompt for a image directory, save a screenshot in clipboard.
Insert an Org link to the image file. The path to the image file
will be changed to the relative path. This is probably necessary
for PowerShell on Windows, as it does not understand \"~/\" as a
valid path."
  (interactive)
  (let* ((dir (file-relative-name
               (read-directory-name  "Choose the image direcotry: "
                                     nil
                                     "images"
                                     'MUSTMATCH "images")))
         (filename (my/take-screenshot dir)))
    (insert (concat "[[file:" filename "]]"))))

(defun my/md-screenshot ()
  "Prompt for a image directory, save a screenshot in clipboard.
Insert a markdown image link. The path to the image file will be
changed to the relative path. This is probably necessary for
PowerShell on Windows, as it does not understand \"~/\" as a
valid path."
  (interactive)
  (let* ((dir (file-relative-name
               (read-directory-name  "Choose the image direcotry: "
